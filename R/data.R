...
#'
#' A collection of small demo inputs for experimenting with functions
#' in the `xmap` package.
#' `_pairs` objects are tibbles with just source-target *pairs* (no weights),
#' `_links` objects are tibbles with weighted source-target *links*, and
#' `_stats` objects are part-to-whole arrays of key-value pairs to transform.
#'
#' @format ## `demo`
#' A list with:
#' \describe{
#'  \item{abc_links}{tibble with 6 rows and 3 columns, specifying links
#'    `lower` -> `upper` by `share`. Covers one-to-one, many-to-one and
#'    one-to-many relations.}
#'  \item{simple_links}{tibble with 10 rows and 3 columns, specifying links
#'    `xcode` -> `alphacode` by `weight`. The running example in
#'    `vignette("applying-crossmaps")`.}
#'  \item{simple_stats}{tibble with 7 rows and 2 columns: a `count` for each
#'    `xcode` source key in `simple_links`, totalling 2800. Conformable with a
#'    crossmap built from `simple_links`, so the two can be used together to
#'    demonstrate `apply_xmap()` end to end.}
#'  \item{aus_state_pairs}{tibble with 8 rows and 2 columns, pairing `ctry`
#'    "AUS" with each Australian state and territory code.}
#'  \item{aus_state_pop_df}{tibble containing 2022 population figures for
#'    Australia by state. Retrieved from:
