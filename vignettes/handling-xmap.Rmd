...
Crossmap tibbles are valid edge lists, and can be visualised as graphs using packages such as [`ggraph`](https://ggraph.data-imaginist.com), or as adjacency matrices. There are two modes for autoplotting `xmap` objects: `bigraph` and `matrix`:
...
## alternatively use "matrix"
autoplot(abc_xmap, plot_type = "matrix")
```