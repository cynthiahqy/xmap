...
* Add "Extracting Crossmaps from Existing Scripts" vignette, with `timor_occupn` and `indstat` (a list of `masked_sample`/`country_lookup`) datasets
...
* `indstat` now covers 8 reporters instead of 5, adding Colombia, Romania and Yemen to the original BRA/CHN/DEU/JPN/USA sample (17,365 rows, years 1990-2013), regenerated from the upstream `xmap-example` export. The three new reporters were chosen for structurally distinct splitting behaviour, and the "Extracting Crossmaps from Existing Scripts" vignette's Case 2 gains a short section using them: composing the extracted `isiccomb -> isic` split with the deterministic `isic -> isic.3` aggregation separates splits that **reconverge** into a single 3-digit parent (imputed at 4 digits, exact at 3 -- Colombia, 0% crossing) from those that **cross** 3-digit boundaries (real propagated allocation uncertainty -- Yemen, 95% crossing), with a forward-reference to `compose_xmap()`. `indstat$country_lookup` is now read from `data-raw/indstat-country-lookup.csv` rather than hardcoded in `data-raw/indstat.R`
