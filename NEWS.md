...
* `vignette("examine-crossmaps")`'s "Applying the collapsed transformation" section now actually runs -- `apply_xmap()` only accepts a single `xmap_tbl`, so applying the per-`country`/`year` `isiccomb -> isic3` crossmap collection needs the same nest-then-map pattern already used later in the vignette (resolves the section's `TODO: implement nested apply_xmap`). The two-step (`split_isiccomb()` then hand-aggregate to `isic3`) comparison used to validate this result now sits immediately after it, instead of further down in "Reconvergent vs. crossing splits"; no change to the reported numbers
